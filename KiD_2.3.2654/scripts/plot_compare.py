...
for j,size in enumerate(convergenceSizes):
  ...
  # All time errors
  qcL2ErrorAllTime[0,j] = np.sqrt(np.sum(np.square(qc0-qcRef))/120*original.dt)
  qcL2ErrorAllTime[1,j] = np.sqrt(np.sum(np.square(qc1-qcRef))/120*original.dt)
  qcL2ErrorAllTime[2,j] = np.sqrt(np.sum(np.square(qc2-qcRef))/120*original.dt)
