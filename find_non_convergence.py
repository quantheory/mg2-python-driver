...
timesteps = np.array([1, 5, 10, 15, 30, 60, 90])
loc_arrays = {
    'T': t_loc,
    'Q': q_loc,
    'QC': qc_loc,
...
worst_slope = 0.
worst_col = -1
worst_col_norms = np.array([])

comp_var = 'Q'

for i in range(total_columns):
    slope, intercept, \
        rvalue, pvalue, stderr = sts.linregress(np.log(timesteps[1:]),
                                                np.log(norms[comp_var][i,:]))
    if np.abs(slope - 1.) > np.abs(worst_slope - 1.):
        worst_slope = slope
        worst_col = i
        worst_col_norms = norms[comp_var][i,:]

print("Worst column is {}.".format(worst_col))
print("Worst slope is {}.".format(worst_slope))
print("Worst norms are {}.".format(worst_col_norms))
