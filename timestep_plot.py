...
#    plt.loglog(timesteps[1:], means, label='mean')
    ...
    plt.loglog(timesteps[1:], np.percentile(norms[name], 95., axis=0), label='95th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 90., axis=0), label='90th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 80., axis=0), label='80th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 70., axis=0), label='70th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 60., axis=0), label='60th percentile')
    plt.loglog(timesteps[1:], medians, label='median')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 40., axis=0), label='40th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 30., axis=0), label='30th percentile')
    plt.loglog(timesteps[1:], np.percentile(norms[name], 20., axis=0), label='20th percentile')
#    plt.legend(loc='best')
    ...
    print("Estimated median convergence rate for variable {}: {}".format(name, (np.log(medians[2]) - np.log(medians[1]))/time_log_diff))
    print("Estimated mean convergence rate for variable {}: {}".format(name, (np.log(means[2]) - np.log(means[1]))/time_log_diff))
